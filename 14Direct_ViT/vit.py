...
class ViTAggregation(nn.Module):
    def __init__(self,n_classes,heads=4,dim_head=32,mlp_dim=128,dim=1024,depth=1,aggr="cls_token",dropout=0.2):
    ...
    def forward(self,inp,return_intermediates=list()):
        ...
        intermediates_tensor=utils_vit.GenericTensorNamespace()
        if self.aggr=="cls_token":
...
class SimpleMLP(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.mean(x, dim=0).unsqueeze(0)                        ## sum over tiles                   
        Y_hat=torch.topk(x,1,dim=1)[1] 
        Y_prob=F.softmax(x,dim=1)
        return  {"logits":x,"Y_prob":Y_prob,"Y_hat":Y_hat}  ## predicted gene_values [n_genes]
# =============================================================================
